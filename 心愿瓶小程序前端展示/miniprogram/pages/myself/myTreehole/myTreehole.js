...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.getStorage({
      ...
      success: function (res) {
        //向后台发送请求，获取用户的心愿信息
        wx.request({
          url: 'http://127.0.0.1:8080/treeholes/' + res.data,
          method: "GET",
          header: {
  ...
  //当用户点击某一条具体的树洞时，跳转到该树洞的详情页面
  toDetail:function(event){
    console.log('正在跳转到树洞详情页面。。。')
    console.log(event.currentTarget.dataset.treeholeid)
    wx.navigateTo({
      //点击某一条树洞跳转到详情页面时，传入对应的树洞id
      url: '../../treehole/treeholeDetail?treeholeId=' + event.currentTarget.dataset.treeholeid 
    })
  },
  ...
  showModal:function(event){
    ...
    this.setData({
      ...
      treeholeid: event.currentTarget.dataset.treeholeid   //把选中的树洞的id设置到变量上
    })
  },
  //用户点击 “确定”，删除对应的树洞信息
  deleteTreehole:function(event){
    var that = this
    var treeholeId = this.data.treeholeid
    wx.request({
